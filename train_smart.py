...
def train(n_episodes, num, epsilon=0.1):
    ...
    for episode in range(n_episodes):
        ...
        if episode % target_update == 0:
            target_dqn.load_state_dict(policy_dqn.state_dict())
            torch.save(policy_dqn.state_dict(), "models/smart/policy-network-{}.pth".format(episode))
...
if __name__ == '__main__':
    ...
    if not os.path.exists("models/smart"):
        os.mkdir("models/smart")
    
    if not os.path.exists("logs"):
        os.mkdir("logs")
    
    if not os.path.exists("logs/sliding-smart"):
        os.mkdir("logs/sliding-smart")
